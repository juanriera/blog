...
No dispongo de datos específicos que expliquen el relanzamiento de 2025, ya que la fábrica de Royal A-Ware estaba en proceso de arranque en 2025 y probablemente no ha consolidado sus volúmenes en esta cifra.

Como veremos al hablar de los quesos DOP europeos, 
...
# --- 3. Mapa ---
p_dop_nuts2 <- ggplot(nuts2_eu) +
  geom_sf(aes(fill = ton_dop), color = "white", linewidth = 0.1) +
  geom_sf(data = nuts2_top15, fill = NA, color = "grey30", linewidth = 0.3) +
  # contorno naranja Asturias
  ...
  annotate("label",
           x = asturias_coords[1] - 00000, y = asturias_coords[2] + 100000,
           label = paste0("Asturias\n", scales::comma(round(
  ...
  theme(
    ...
    # Reducción del tamaño de letra
    legend.title = element_text(size = 9, face = "bold"), # Título más pequeño
    legend.text = element_text(size = 8),                # Números más pequeños
...
Fuente: MAPA, INAO, CLAL, DOOR, TSM
Datos Suiza (Gruyère, Emmental, Appenzeller, Raclette du Valais, Sbrinz):\nestimaciones aproximadas a partir de fuentes secundarias (TSM/Agristat, ca. 2022)
Elaboración propia"
  )
...
asturias_rows <- tibble(
  ...
  ton_dop      = c(35310, 670),
  quesos       = c("-", "Afuega'l Pitu, Cabrales, Gamonéu, Casín, Los Beyos")
)
