...
ggplot() +
  ...
  geom_text_repel(
    data = centroides_laea |> filter(nuts2_nombre == "Région lémanique"),
    ...
    box.padding   = 0.1,
    point.padding = 0.1,
    nudge_x       = -300000,   # ajusta hasta que quede bien
    nudge_y       =   100000,
    direction     = "x",
    force         = 0,
    force_pull    = 0.5,
  ...
  geom_text_repel(
    data = centroides_laea |> filter(nuts2_nombre == "Espace Mittelland"),
    aes(label = paste0(nuts2_nombre, "\n", scales::comma(round(ton_dop)), " t"),
        geometry = geometry),
    stat          = "sf_coordinates",
    size          = 2.2,
    ...
    box.padding   = 0.1,
    point.padding = 0.1,
    nudge_x       = -30000,   # ajusta hasta que quede bien
    nudge_y       = +150000,
    direction     = "x",
    force         = 0,
    force_pull    = 0.5,
  ...
  labs(caption  = "Producción distribuida equitativamente entre regiones NUTS2 del ámbito geográfico de cada DOP
Datos Suiza (Gruyère, Emmental, Appenzeller, Raclette du Valais, Sbrinz): estimaciones aproximadas (TSM/Agristat, ca. 2022)
...
quesos_por_region <- dop_raw %>%
  mutate(ton_asignada = ton / n_regiones) %>%
  arrange(nuts2_code, desc(ton_asignada)) %>%
  group_by(nuts2_code) %>%
...
gt_top10 <- gt_data %>%
  ...
  tab_source_note(
    "Producción distribuida equitativamente entre regiones NUTS2 del ámbito geográfico de cada DOP
Asturias total: SADEI 2026 · Asturias DOP/IGP: MAPA 2023
