...
# --- 4. Tabla gt top 15 más asturias ---
dop_nuts2 <- dop_raw %>%
  mutate(ton_asignada = ton / n_regiones) %>%
  group_by(nuts2_code, nuts2_nombre) %>%
  summarise(ton_dop = sum(ton_asignada, na.rm = TRUE), .groups = "drop") %>%
...
gt_data <- bind_rows(top15, asturias_rows) %>%
  mutate(ranking = c(as.character(1:15), "-", "-")) %>%
  select(ranking, pais, nuts2_nombre, nuts2_code, ton_dop)
...
Resto: MAPA, INAO, CLAL, DOOR · elaboración propia"
  ...
  tab_options(
    ...
    heading.align = "left",
    column_labels.font.weight = "bold",
    table.width       = px(540)   # suma aproximada de tus columnas: 40+80+200+80+120+márgenes
  )
