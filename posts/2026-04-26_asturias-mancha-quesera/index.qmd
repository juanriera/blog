...
df_plot <- df_paises %>%
  ...
  mutate(
    ...
    pais = reorder(pais, empresas_km2)
  )
...
# 3. Gráfico
ggplot(df_plot, aes(x = pais, y = empresas_km2, fill = highlight)) +
  geom_col() +
  scale_fill_manual(values = c("Otros" = "grey60", 
                               "España" = "#1f77b4", 
                               "Asturias" = "#D55E00")) +
...
df_plot <- df_paises %>%
  ...
  mutate(
    ...
    empresas_km2 = num_empresas_queseras / (superficie_km2/1000),
    empresas_DOP_km2 = num_empresas_queseras_DOP / (superficie_km2/1000),
    pais = reorder(pais, empresas_DOP_km2)
  )
...
# 3. Gráfico
ggplot(df_plot, aes(x = pais, y = empresas_DOP_km2, fill = highlight)) +
  geom_col() +
  scale_fill_manual(values = c("Otros" = "grey60", 
                               "España" = "#1f77b4", 
                               "Asturias" = "#D55E00")) +
...
df_paises %>%
  ...
  mutate(highlight = case_when(
      ...
      empresas_km2 = num_empresas_queseras / (superficie_km2/1000),
      empresas_DOP_km2 = num_empresas_queseras_DOP / (superficie_km2/1000)) %>%
  ggplot(aes(x = empresas_km2, y = empresas_DOP_km2)) +
  # 2. Puntos: Gris uniforme para 'Otros' y colores para España/Asturias
  geom_point(aes(color = highlight), size = 2.5, alpha = 0.7) +
  # 3. Etiquetas: Solo ponemos nombre a los puntos destacados para evitar ruido
  # geom_text_repel(aes(label = if_else(highlight == "Otros", "", pais), color = highlight),
...
Aunque la producción de queso en Asturias es importante, no es en ningún caso superior a la de otras zonas de Europa, comparando con países completos. Si buscásemos zonas queseras importantes de Europa, seguramente la reducción de la superficie considerada haría subir las ratios para estas zonas de alta producción.  Por ello no parece justificado denominar a Asturias ***la mayor mancha quesera de Europa***, aunque es necesario mejorar la información regional europea para validar esta conclusión. Por otra parte, en el total de queso fabricado en Asturias hay una cantidad importante de productos que no tienen una vinculación con la región más allá de estar fabricados en ella: es el caso de mozzarella, quesos fundidos y otros, que la última edición del **Catálogo general de los quesos elaborados en Asturias** @vadequesum2026 ya ha excluido del catálogo. Resulta clave elaborar un plan estratégico que defina y refuerce los atributos distintivos del queso asturiano y apoye tanto a los quesos con certificación DOP e IGP como a los quesos de autor de alta calidad, diferenciando con claridad los quesos catalogados de los quesos artesanos, y facilitando el desarrollo de iniciativas de consolidación que permitan aumentar el tamaño de las queserías asturianas. En este sentido, estudiar otros modelos regionales, como el suizo, puede ser de gran interés.
