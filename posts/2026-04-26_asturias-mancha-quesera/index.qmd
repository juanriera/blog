...
La @tbl-tabla-asturias presenta los datos de la producción total de queso en Asturias en los últimos quince años, actualizada a 2025, según la Sociedad Asturiana de Estudios Económicos e Industriales (SADEI)[@sadei].
...
La producción de queso en Asturias se mantuvo en torno a 35.000-40.000 toneladas desde 2010 hasta 2022. Entre 2021 y 2023 se registra una caída notable debido principalmente al cierre de la fábrica de Danone en Salas en 2022[@lva2022-2], que traslada la elaboración del queso fresco *Danonino* a otros centros en Francia. A partir de 2023, la producción regional se estabiliza en unas 25.000 toneladas, lo que representa un 4,6% del total nacional,  y en 2025 remonta a 35.310 toneladas. Este volumen incluye productos industriales de gran volumen, como el queso fresco *Burgo de Arias* y otros quesos frescos fabricados por Mantequerías Arias en su planta de Vegalencia (Soto de Ribera), la mozzarella que fabrica Reny Picot en Anleo (Navia) y los quesos mascarpone y mozzarella fabricados por Quesería Lafuente en Arriondas. Si descontamos estos volúmenes del total dado por SADEI, podemos situar el volumen de los quesos tradicionales asturianos en torno a las 10.000 toneladas, incluyendo los quesos DOP e IGP, como veremos en el capítulo dedicado a los quesos DOP europeos.

El detalle de la producción quesera asturiana fuera de las denominaciones de origen no se recoge en ningún informe oficial. Es el caso de quesos importantes y conocidos, algunos tradicionales como Vidiago, Pría, La Peral, Peñamellera, Abredo y otros, y también de los quesos que podrían ser incluidos en los llamados quesos *de autor*, como Rey Silo, Lazana, Varé o La Chivita, y multitud de quesos artesanos producidos en pequeñas queserías, cuyo volumen me ha resultado imposible de estimar con los datos de que dispongo.

La @fig-prod-asturias muestra la producción de queso desde 1990. En el período 2005 a 2010 se identifica una caída importante de los volúmenes de producción, probablemente reflejo de varios factores, como la crisis de 2008, unida al efecto de la reordenación de la PAC entre 2003 y 2009 y a la relocalización por Mantequerías Arias de una parte importante de la fabricación de Burgo de Arias en la nueva factoría de Burgos, hacia 2006. También se ve claramente el cese de actividad de Danone en Salas en 2023. 

No dispongo de datos específicos que expliquen el relanzamiento de 2025, ya que la fábrica de Royal A-Ware estaba en proceso de arranque en 2025 y probablemente sus volúmenes no están consolidados en el valor de este año.
...
eu <- ne_countries(scale = "medium", returnclass = "sf") %>%
  filter(region_un == "Europe" | name == "Turkey") %>%
  rename(pais = name) %>%
...
p_europa <- ggplot(eu_data) +
  geom_sf(aes(fill = prod_total_queso_ton), color = "grey70", linewidth = 0.2) +
  geom_sf(fill = NA, color = "grey70", linewidth = 0.4) +
...
theme_void() +
  theme(
    ...
    # x = 0.02 (muy pegado al borde izquierdo), y = 0.2 (bajo)
    legend.position.inside = c(0.02, 0.55),
    # Anclamos la leyenda por su esquina inferior izquierda
    legend.justification = c(0, 0),
    
    # Reducción del tamaño de letra
    legend.title = element_text(size = 7, face = "bold"), # Título más pequeño
    legend.text = element_text(size = 6),                # Números más pequeños
    
    # Reducimos también el tamaño de la clave (el cuadradito de color) 
    # para que toda la leyenda sea más compacta
    legend.key.size = unit(0.3, "cm"),
    
    # Fondo minimalista
    legend.background = element_rect(fill = alpha("white", 0.6), color = NA),
    
    # ELIMINA LOS MÁRGENES DEL PLOT
...
centroides_pct <- st_centroid(st_crop(eu_pct, xmin = -25, ymin = 34, xmax = 45, ymax = 72))

p_pct_dop <- ggplot(eu_pct) +
  geom_sf(aes(fill = pct_dop), color = "grey70", linewidth = 0.2) +
  geom_sf(fill = NA, color = "grey70", linewidth = 0.4) +
  geom_sf_text(data = centroides_pct,
               aes(label = ifelse(!is.na(pct_dop), 
                                  paste0(round(pct_dop, 0), "%"), 
                                  "")),
               size = 2.5, 
               color = "grey30",
               fontface = "bold") +
...
## La producción DOP/IGP en las principales regiones queseras de Europa

El mapa y la tabla a continuación muestran la producción de Asturias en relación con la producción europea de queso DOP/IGP. Para situar mejor el contexto, en la tabla he añadido los valores de producción total de Asturias. 

Con el fin de ajustar este total de forma más realista, se podrían restar unas 4.000 ton para Mantequerías Arias y 5.000 para Quesería Lafuente, así como unas 15.000 ton para la fabricación de mozzarella de Reny Picot; la producción de mozzarella por Royal A-Ware en Salas está arrancando en 2025-2026 y probablemente no figura en la cifra de SADEI de 2025. 

La cifra aproximada sin las principales producciones industriales sería de unas 10 mil ton, que incluiría:

- la producción de pequeños industriales como Quesos Monteverde, Quesos La Peral, Quesos Ines Granda, Los Caserinos y otros, 
- La producción DOP/IGP que está en torno a 700 ton
- Otros pequeños artesanos y quesos de autor

Esta cifra de producción situaría el total de Asturias en la tabla comparada de producción DOP en torno a la posición 30. Dicho de otra forma, la producción total de Asturias, sin sus principales quesos industriales, es inferior a la producción sólo DOP/IGP de treinta de las principales zonas queseras de Europa. Hay que tener en cuenta, además, que hay otros países con una producción importante (caso de Países Bajos, con una producción de Gouda Holland en torno a 300.000 ton) que no he podido confirmar con detalle y he preferido dejar fuera de este mapa.

Es interesante destacar que una parte muy importante de la producción quesera DOP europea se concentra en las  zonas montañosas de Francia, Italia y Suiza, de manera semejante a lo que sucede en Asturias en torno a los Picos de Europa. Hay que preguntarse si esta producción de quesos de montaña asturianos tienen el apoyo suficiente para desarrollarse adecuadamente.

```{r}
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| label: fig-mapa-europa
#| fig-cap: "Producción de queso DOP/IGP por región europea (regiones principales)"
#| out-width: "100%"

# --- 1. Datos DOP ---

dop_raw <- read_csv2("dop_nuts2.csv", locale = locale(encoding = "latin1"))

dop_nuts2 <- dop_raw %>%
  group_by(nuts2_code, nuts2_nombre) %>%
  summarise(ton_dop = sum(ton, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(ton_dop))

top15 <- dop_nuts2 %>% slice_head(n = 15)

# --- 2. Geometrías NUTS2 ---
# guardadas previamente con
# nuts2 <- gisco_get_nuts(year = "2021", epsg = "4326", resolution = "10", nuts_level = "2")
# st_write(nuts2, "datos/nuts2_2021.gpkg", delete_dsn = TRUE)

nuts2 <- st_read("datos/nuts2_2021.gpkg", quiet = TRUE) %>%
  rename(NUTS_ID = id) %>%
...
nuts2_eu <- nuts2 %>%
  ...
  left_join(dop_nuts2, by = c("NUTS_ID" = "nuts2_code"))

nuts2_top15 <- nuts2_eu %>%
  filter(NUTS_ID %in% top15$nuts2_code)

centroides_top15 <- st_centroid(nuts2_top15)

laea_crs <- "+proj=laea +lat_0=52 +lon_0=10 +x_0=4321000 +y_0=3210000 +ellps=GRS80"
centroides_laea <- st_transform(centroides_top15, crs = laea_crs)

asturias_pt <- st_sfc(st_point(c(-5.86, 43.36)), crs = 4326)
asturias_laea <- st_transform(asturias_pt, crs = laea_crs)
asturias_coords <- st_coordinates(asturias_laea)

paises_eu <- ne_countries(scale = "medium", returnclass = "sf")

# --- 3. Mapa ---
ggplot() +
  geom_sf(data = nuts2_eu, aes(fill = ton_dop), color = "grey85", linewidth = 0.1) +
  scale_fill_distiller(
    palette   = "YlGnBu", direction = 1,
    name      = "Producción DOP/IGP\n(toneladas)",
    na.value  = "grey95", labels = scales::comma
  ) +
  geom_sf(data = nuts2_top15, fill = NA, color = "grey50", linewidth = 0.3) +
  geom_sf(data = nuts2_eu %>% filter(NUTS_ID == "ES12"),
          fill = NA, color = "#D55E00", linewidth = 0.5, alpha = 0.8) +
  geom_text_repel(
  # data = centroides_laea |> filter(!nuts2_nombre %in% c("Région lémanique","Espace Mittelland")),
    data = centroides_laea,
    aes(label = paste0(nuts2_nombre, "\n", scales::comma(round(ton_dop)), " t"),
        geometry = geometry),
    stat          = "sf_coordinates",
    size          = 2.2,
    color         = "grey30",
  ...
  geom_sf(data = paises_eu,
          fill = NA,
          color = "grey70",
          linewidth = 0.5) +
  annotate("text",
           x = asturias_coords[1]-50000, y = asturias_coords[2] + 100000,
           label = paste0("Asturias\n  ", scales::comma(round(
             dop_nuts2 %>% filter(nuts2_code == "ES12") %>% pull(ton_dop)
           )), " t"),
           size = 2.2, color = "#D55E00",
           hjust = 0, fontface = "bold")+
  coord_sf(crs  = laea_crs,
           xlim = c(2200000, 5800000), ylim = c(1500000, 3100000)) +
  theme_void() +
  theme(
    legend.position        = "inside",
    legend.position.inside = c(0.02, 0.55),
  ...
  labs(caption  = "Datos Suiza (Gruyère, Emmental, Appenzeller, Raclette du Valais, Sbrinz): estimaciones aproximadas (TSM/Agristat, ca. 2022)
División administrativa regional: NUTS 2024 Eurostat/GISCO
Fuente: MAPA, INAO, CLAL, DOOR, TSM · elaboración propia"
  )
```
...
#| warning: FALSE
#| message: FALSE
#| label: tbl-mapa-europa
#| fig-cap: ""

quesos_por_region <- dop_raw %>%
  arrange(nuts2_code, desc(ton)) %>%
  group_by(nuts2_code) %>%
  summarise(quesos = paste(queso, collapse = ", "), .groups = "drop")

top10 <- dop_nuts2 %>%
  slice_head(n = 10) %>%
  left_join(dop_raw %>% distinct(nuts2_code, pais), by = "nuts2_code") %>%
  left_join(quesos_por_region, by = "nuts2_code")

asturias_rows <- tibble(
  ranking      = c("-", "-"),
  pais         = c("España", "España"),
  nuts2_nombre = c("Asturias - Total", "Asturias - DOP/IGP"),
  nuts2_code   = c("ES12", "ES12"),
  ton_dop      = c(35310, 670),
  quesos       = c("Incluye unas 25.000 ton de quesos industriales como mozzarella, mascarpone y queso fresco tipo Burgos", "Afuega'l Pitu, Cabrales, Gamonéu, Casín, Los Beyos")
)

gt_data <- bind_rows(top10, asturias_rows) %>%
  mutate(ranking = c(as.character(1:10), "-", "-")) %>%
  select(ranking, pais, nuts2_nombre, nuts2_code, ton_dop, quesos)
...
gt_top10 <- gt_data %>%
  ...
  cols_label(
    ranking      = "#",
    pais         = "País",
    nuts2_nombre = "Región NUTS2",
    nuts2_code   = "Código",
    quesos = "Quesos DOP/IGP",
    ton_dop      = "Producción DOP (t)"
  ) %>%
  cols_width(
    ranking      ~ px(20),
    pais         ~ px(60),
  ...
  fmt_number(columns = ton_dop, decimals = 0, sep_mark = ".", dec_mark = ",") %>%
  data_color(columns = ton_dop, palette = "Blues") %>%
  tab_style(
    style = list(cell_text(weight = "bold", color = "#D55E00")),
    locations = cells_body(rows = nuts2_code == "ES12")
  ) %>%
  tab_header(
    title    = "Principales regiones europeas productoras de queso DOP/IGP",
    subtitle = "Top 10 por volumen de producción asignado · España, Francia, Italia, Suiza y Grecia"
  ) %>%
  tab_source_note("Asturias total: SADEI 2026 · Asturias DOP/IGP: MAPA 2023
  División administrativa regional: NUTS 2024 Eurostat/GISCO
  Resto: MAPA, INAO, CLAL, DOOR · elaboración propia"
  ) %>%
  tab_options(
    table.font.size = 12,
    heading.align = "left",
...
Antes de presentar el análisis de estas ratios, es importante señalar que las ratios de producción por habitante o por $km^2$ en las regiones queseras europeas que hemos visto son significativamente más altas, al concentrar la producción en un área más reducida que sus países respectivos completos(ver @tbl-mapa-europa). He usado las ratios por países para dar una visión general, que creo que sigue siendo válida y permite comparar con países para los que no he podido obtener datos regionales detallados.
...
p_kg_hab <- ggplot(eu_kg) +
  geom_sf(aes(fill = kg_queso_habitante), color = "grey70", linewidth = 0.2) +
  geom_sf(fill = NA, color = "grey70", linewidth = 0.4) +
...
p_km2 <- ggplot(eu_km2) +
  geom_sf(aes(fill = ton_tot_km2), color = "grey70", linewidth = 0.2) +
  geom_sf(fill = NA, color = "grey70", linewidth = 0.4) +
...
p_emp <- ggplot(eu_emp) +
  geom_sf(aes(fill = ton_queso_por_empresa), color = "grey70", linewidth = 0.2) +
  geom_sf(fill = NA, color = "grey70", linewidth = 0.4) +
...
p_emp_km2 <- ggplot(eu_emp_km2) +
  geom_sf(aes(fill = empresas_1000km2), color = "grey70", linewidth = 0.2) +
  geom_sf(fill = NA, color = "grey70", linewidth = 0.4) +
