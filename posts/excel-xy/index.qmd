...
Utilizaremos el dataset `iris`([Conjunto de datos Iris en Wikipedia](https://es.wikipedia.org/wiki/Conjunto_de_datos_flor_iris), [Conjunto de datos Iris en kaggle](https://www.kaggle.com/datasets/uciml/iris)), exportado desde R como CSV, que convertimos en una tabla de Excel mediante `Menu`\>`Insertar`\>`Tabla`
...
Excel siempre utiliza fórmulas de agrupación en los campos de valores, y por defecto utiliza `Recuento`. La fórmula de `Recuento` nos permite ver que hay algunas parejas de valores que están repetidas, por ejemplo, hay 8 valores de `Petal.Width`que coinciden con un valor de 5 en `Sepal.Length` y que la tabla dinámica ha agrupado. Esto es un inconveniente porque en un gráfico de dispersión queremos que nos aparezcan todos los pares de valores sin agrupar, para que la nube de puntos represente el total de los datos.
...
Este es un gráfico de dispersión correcto, en el que la escala de los ejes X e Y respeta la distancia que indica la magnitud del valor numérico, y por lo tanto, sitúa los puntos en una posición correcta. Este gráfico de dispersión, en el que los valores Y están resumidos en un valor único, puede ser útil en algunas circunstancias; nos simplifica la representación gracias a que la fórmula `Promedio`nos reduce a un punto todos los valores de Y que se corresponden con un valor determinado de X. Nos permite visualizar claramente las diferencias entre las tres poblaciones que estamos representando; sin embargo, no es el gráfico que buscamos, porque no hemos representado cada par de valores que existe en la tabla de datos original, estén repetidos o no.
...
-   Con el cursor en una celda en blanco fuera de la tabla dinámica y a la derecha, insertamos gráfico de dispersión en blanco,
...
He añadido en YouTube un [vídeo](https://youtu.be/tD2P5wabb1k) con la secuencia de creación del gráfico.

La tabla de datos *Iris* puede descargarse enn el repositorio [datasets](https://github.com/juanriera/datasets) de mi GitHub.

El gráfico exportado a PDF puede descargarse [aquí](https://github.com/juanriera/blog/blob/3e356214c7a34a908fb6b2e828c97750a98e13e1/galeria/excel-graf-XY-final.pdf) para verificar la calidad que puede obtenerse mediante una exportación desde Excel.

Una hoja de cálculo Excel con todos los gráficos puede descargarse [aquí](iris_fisher.xlsx) o en el repositorio [datasets](https://github.com/juanriera/datasets) de mi GitHub.
