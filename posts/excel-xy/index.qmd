---
title: "Cómo hacer gráficos XY multiserie en Microsoft Excel a partir de datos arreglados (*tidy data*)"
author: "Juan Riera"
date: 2023-01-28
format: html
execute: 
...
Excel llama **gráfico XY** o **gráfico de dispersión** al que tiene un eje X numérico que representa valores y no categorías. En contraposición, un **gráfico de líneas** muestra el eje X como categorías, en la secuencia en la que aparecen los valores en la hoja de cálculo, sin importar la magnitud de su valor.
...
Excel tiene la opción de hacer gráficos de dispersión para dos variables, que deben estar en dos columnas. Una de las cosas que siempre he echado de menos en este programa, y que es muy fácil en R o python, es la posibilidad de hacer gráficos XY multiserie, segmentados por una columna adicional que se utiliza para codificar en color de acuerdo con los valores de esa columna. Con `ggplot` en R, es inmediato utilizar `colour` para las categorías:
...
## Cómo hacer un gráfico XY multiserie (dispersión) en Excel
...
Queremos hacer una gráfica XY de la columna `Sepal.Length` frente a `Petal.Width`, agrupada por `Species`, igual que hemos visto en el gráfico hecho con `ggplot`.
...
Para conseguir un gráfico XY verdadero, empezamos insertando un gráfico XY en blanco a la derecha de la tabla dinámica. Para ello, colocamos el cursor dos columnas a la derecha de la tabla dinámica e insertamos un nuevo gráfico XY, que nos aparecerá como un marco en blanco, ya que no hemos establecido ningún rango de datos. Esto es lo que haremos a continuación.
