...
Hay un dato que todo quesero registra y pocos analizan con la profundidad que merece: la curva de descenso del pH durante la elaboración. No es un simple indicador de control — es la firma dinámica del fermento y la huella del proceso: refleja la actividad real de los microorganismos, y su perfil de descenso determina el grado de desmineralización, que tiene una gran influencia en la textura final. La modelización matemática de curvas microbianas es un campo con raíces en los años noventa [@Zwietering1990; @Baranyi1994] que sigue siendo activo y relevante — tanto en microbiología predictiva como en tecnología quesera [@Dey2025]. En este artículo explico cómo aplicar esa tradición a la curva de acidificación, qué información extraemos de sus parámetros y cómo elegir entre modelos según los datos disponibles.
...
Los parámetros A, K, tᵢ y r tienen exactamente el mismo significado tecnológico que en la logística. La diferencia es estructural: en Gompertz, el punto de inflexión ocurre en tᵢ — igual que en todos los modelos de esta familia — pero en ese instante se ha completado el **63,2%** del descenso total (= 1 − 1/e). Esto implica que la curva de Gompertz es asimétrica: la caída antes de tᵢ es lenta y prolongada, y la aceleración se concentra en el tramo final hasta K. Gompertz asume implícitamente ese perfil para cualquier cepa, sin posibilidad de ajuste.
...
![Curva de acidificación — modelo de Gompertz. El punto de inflexión ocurre en tᵢ pero en ese instante se ha completado el 63,2% del descenso total (línea de puntos). La asimetría es la inversa al perfil biológico habitual de las cepas lácticas.](imagenes/02-mod-gompertz.png)

Comparando ambas figuras, la diferencia de asimetría es visible: la curva de Gompertz tiene una fase inicial más plana y concentra la caída en la segunda mitad del descenso. En muchas cepas lácticas ese perfil no describe bien la realidad — el fermento alcanza su máxima actividad relativamente pronto, con una caída inicial rápida seguida de una larga cola. Esa asimetría inversa es exactamente lo que Gompertz no puede representar, porque la proporción del 63,2% completada en tᵢ es una constante matemática intrínseca al modelo, no un parámetro ajustable.

Gompertz lleva más de tres décadas siendo el estándar en microbiología predictiva desde el trabajo fundacional de Zwietering et al. (1990) [@Zwietering1990], pero conviene precisar el contexto: esa literatura lo aplica a curvas de **crecimiento** bacteriano, que tienen una dinámica opuesta a la acidificación — fase lag larga, aceleración tardía y autolimitación brusca al agotar el sustrato. Esa forma, con el 63,2% del recorrido completado en tᵢ, describe bien el crecimiento pero no la acidificación láctica, donde el fermento arranca con actividad alta y se frena de forma progresiva. Aplicar Gompertz a curvas de acidificación no es solo una simplificación: introduce implícitamente una asimetría que contradice la biología del proceso. Los datos de ajuste de las cepas Danisco/IFF lo confirman empíricamente — la logística supera a Gompertz en cinco de los seis casos con los mismos cuatro parámetros.
...
En esta parametrización, **tᵢ es siempre el tiempo de máxima velocidad de descenso**, independientemente de ν — es un parámetro con significado biológico directo y observable en el gráfico antes de ajustar el modelo. Lo que ν controla es la fracción del descenso total completada en ese instante, es decir, la asimetría de la curva a cada lado de tᵢ:

- **ν → 0**: el modelo converge a Gompertz. En tᵢ se ha completado el 63,2% del descenso. La curva tiene arranque lento y caída concentrada en la segunda mitad — la mayor parte del descenso ocurre *después* del momento de máxima velocidad.
- **ν = 1**: la expresión se reduce a la logística estándar. En tᵢ se ha completado el 50%. La curva es simétrica.
- **ν > 1**: en tᵢ se ha completado menos del 50% del descenso. La curva tiene arranque rápido y una larga cola de inhibición — la mayor parte del descenso ocurre *antes* del momento de máxima velocidad.
...
Es importante distinguir qué controla cada parámetro y qué variables de proceso pueden modificarlo. **r y tᵢ** son sensibles a las condiciones de trabajo: aumentar la temperatura de cuba incrementa r, lo que adelanta tᵢ y eleva Vmax. La dosis de inoculación actúa de forma diferente y más compleja. Lo que la observación empírica muestra [@Mietton1994] es que la Vmax — la pendiente máxima en tᵢ — se mantiene aproximadamente constante al variar la dosis para la misma cepa y temperatura: doblar la dosis equivale a ganar aproximadamente un tiempo de generación bacteriana, adelantando el momento de máxima actividad sin modificar su intensidad. Este comportamiento tiene respaldo en la literatura de microbiología predictiva: en comparaciones directas entre modelos ajustados a distintas concentraciones de inóculo, la tasa de crecimiento intrínseca de la cepa — el parámetro r en nuestro contexto, que juega el papel análogo al µ de la literatura de crecimiento bacteriano [@Buchanan1997] aunque aplicado a un proceso de descenso en lugar de crecimiento — permanece aproximadamente constante, mientras que la duración de la fase de latencia disminuye al aumentar la concentración del inóculo. Sin embargo, esto no significa que las curvas sean desplazamientos idénticos de la misma sigmoide. La forma de la curva cambia con la dosis: con dosis alta el arranque es más brusco (la fase de latencia es más corta y la transición hacia la caída activa más abrupta); con dosis baja el arranque es más gradual. Solo en la zona central alrededor de tᵢ las curvas son verdaderamente paralelas. Los modelos sigmoidales de cuatro o cinco parámetros describen bien cada curva individualmente una vez ajustada, pero no capturan analíticamente cómo cambia la forma completa de la curva con la dosis — para eso sería necesario un modelo con fase de latencia explícita, como el modelo de Baranyi [@Baranyi1994; @Buchanan1997]. **ν**, en cambio, es una característica intrínseca de la cepa — describe cómo distribuye su actividad a lo largo del descenso, independientemente de la escala temporal. Dos lotes del mismo fermento a distinta temperatura tendrán tᵢ y Vmax diferentes pero un ν similar; dos cepas distintas con el mismo tᵢ y la misma Vmax pueden tener ν muy distintos y producir perfiles de desmineralización diferentes, con consecuencias organolépticas que los parámetros básicos no capturan.
