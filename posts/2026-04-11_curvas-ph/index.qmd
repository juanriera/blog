...
En esta parametrización, **$t_i$ es siempre el tiempo de máxima velocidad de descenso**, independientemente de ν — es un parámetro con significado biológico directo y observable en el gráfico antes de ajustar el modelo. Lo que ν controla es la fracción del descenso total completada en ese instante, es decir, la asimetría de la curva a cada lado de $t_i$:

- **ν → 0**: el modelo converge a Gompertz. En $t_i$ se ha completado el 63,2% del descenso. La curva tiene arranque lento y caída concentrada en la segunda mitad — la mayor parte del descenso ocurre *después* del momento de máxima velocidad.
- **ν = 1**: la expresión se reduce a la logística estándar. En $t_i$ se ha completado el 50%. La curva es simétrica.
- **ν > 1**: en $t_i$ se ha completado menos del 50% del descenso. La curva tiene arranque rápido y una larga cola de inhibición — la mayor parte del descenso ocurre *antes* del momento de máxima velocidad.
...
Es importante distinguir qué controla cada parámetro y qué variables de proceso pueden modificarlo. **r y $t_i$** son sensibles a las condiciones de trabajo: aumentar la temperatura de cuba incrementa r, lo que adelanta $t_i$ y eleva Vmax. La dosis de inoculación actúa de forma diferente y más compleja. Lo que la observación empírica muestra [@Mietton1994] es que la Vmax — la pendiente máxima en $t_i$ — se mantiene aproximadamente constante al variar la dosis para la misma cepa y temperatura: doblar la dosis equivale a ganar aproximadamente un tiempo de generación bacteriana, adelantando el momento de máxima actividad sin modificar su intensidad. Sin embargo, esto no significa que las curvas sean desplazamientos idénticos de la misma sigmoide. La forma de la curva cambia con la dosis: con dosis alta el arranque es más brusco (la fase de latencia es más corta y la transición hacia la caída activa más abrupta); con dosis baja el arranque es más gradual. Solo en la zona central alrededor de $t_i$ las curvas son verdaderamente paralelas. Los modelos sigmoidales de cuatro o cinco parámetros describen bien cada curva individualmente una vez ajustada, pero no capturan analíticamente cómo cambia la forma completa de la curva con la dosis — para eso sería necesario un modelo con fase de latencia explícita, como el modelo de Baranyi. **ν**, en cambio, es una característica intrínseca de la cepa — describe cómo distribuye su actividad a lo largo del descenso, independientemente de la escala temporal. Dos lotes del mismo fermento a distinta temperatura tendrán $t_i$ y Vmax diferentes pero un ν similar; dos cepas distintas con el mismo $t_i$ y la misma Vmax pueden tener ν muy distintos y producir perfiles de desmineralización diferentes, con consecuencias organolépticas que los parámetros básicos no capturan.

![Misma cepa (ν = 0,5), distinto $t_i$ por condiciones de proceso. Las dos curvas comparten A, K y ν — la línea de puntos horizontal marca la fracción del descenso completada en $t_i$, idéntica en ambos casos (55,6%). Lo que cambia es $t_i$ (adelantado de 10 h a 7 h por mayor temperatura o dosis) y Vmax (aumentada de 0,213 a 0,308 pH/h). La asimetría intrínseca de la cepa es independiente de las condiciones de proceso.](imagenes/07-richards-nu-proceso.png)
...
![Efecto del parámetro ν en el modelo de Richards parametrizado por tiempo de inflexión. Las tres curvas comparten A, K, r y $t_i$ = 10 h. Los círculos marcan el punto de inflexión, que siempre ocurre en $t_i$ para los tres valores de ν. Lo que cambia es el pH en ese instante — es decir, la fracción del descenso ya completada — y con ello la forma de la curva a cada lado: ν = 0,5 concentra la caída después de $t_i$ (55,6% completado); ν = 1,0 es simétrica (50%); ν = 1,5 concentra la caída antes de $t_i$ (45,7% completado, arranque rápido).](imagenes/03-mod-richards-nu.png)
...
**Cepa MA** — ν oscila entre 0,36 (a 37°C) y 1,45 (a 25°C). En $t_i$ se ha completado entre el 44% y el 56% del descenso según la temperatura — la curva es relativamente simétrica, lo que explica que la logística la ajuste casi tan bien como Richards. Vmax es alta: 1,18–1,53 pH/h.
