...
## Cómo hacerlo en R
Adjunto el código R para hacer un gráfico equivalente al anterior en R. El posicionamiento de las imágenes es más engorroso; hay que detallar las coordenadas de cada imagen. El resultado definitivo sólo se consigue por ensayo y error, al no ser posible la edición visual de la que dispone Excel. Sin embargo, tiene la ventaja de que todas las opciones que se han usado en la configuración del gráfico están recogidas en el código, lo que hace que sea totalmente reproducible.
...
scat <-
  ...
  annotation_custom(i1, ymin = 45, ymax = 65, xmin = 235, xmax =280) +
  annotation_custom(i2, ymin = 20, ymax = 55, xmin = 230, xmax =280) +
  annotation_custom(i3, ymin = 63.5, ymax = 70, xmin = 260, xmax =280) +
