...
En resumen,
Power Query / M se utiliza para la ingeniería de datos:

- ***Data Ingestion***: Power Query permite importar, combinar y actualizar datos de varias fuentes (por ejemplo, desde una carpeta local, SharePoint, base de datos, API, Azure y muchos más).
- **Procesos ETL y ELT**: Extraer–Transformar–Cargar y Extraer–Cargar–Transformar son los dos enfoques para el procesamiento de datos. En ambos casos, Power Query es la herramienta a utilizar.
- **Gestión de datos**: Limpieza y transformación de los datos. Se realizan uniones, se filtran, se asignan nombres uniformes, se eliminan columnas innecesarias, etc.

Se aplican las siguientes reglas:

- Los conjuntos de datos que no vamos a necesitar en el análisis de datos se filtran en M
- No agregamos registros en M
- Las columnas que no vamos a necesitar se eliminan en M
- En general, no hacemos cálculos en M

DAX se utiliza para el análisis de datos, por tanto, para realizar:

- **Agregaciones**: si necesitamos recuperar valores agregados de una tabla o un conjunto de datos, la forma mejor y más rápida de hacerlo es con las funciones de agregado DAX.
- **Filtrado**: en las medidas, las funciones de filtrado se encuentran entre las más utilizadas. Ayudan a filtrar solo la información relevante y/o editar el contexto de datos para crear cálculos dinámicos sin cambiar el modelo de datos y sus tablas. Esto es importante para visualizar y agregar valores específicos para informes y paneles y otros cálculos en Power BI, utilizando estas métricas de filtro.
- **Definir relaciones**: Power Query no conoce relaciones. En DAX, por otro lado, puede definir y cambiar las relaciones, pero también aplicarlas mediante la integridad referencial.
- **Funciones de relación (jerarquías)**. 
- **Columna calculada que hace referencia a una columna de otra tabla**. Con DAX, el usuario puede insertar fácilmente columnas de diferentes tablas en la fórmula. En Power Query, esta maniobra requiere uniones complejas y se complica muy rápidamente.
...
Esta es la fórmula corregida: `=IF(ISBLANK(Tabla1[bill_length_mm]);BLANK();Tabla1[bill_depth_mm]+CALCULATE(RAND()/100))`
que devuelve un valor en blanco cuando el valor original es un valor en blanco.
...
![Comparación de la longitud de la aleta vs la longitud del pico en el dataset Pinguinos Palmer](pinguinos.png)
